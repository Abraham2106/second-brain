...
# Centralized path for the SQLite database.
# Points to 'ai_team.db' in the project root.
_base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../.."))
DB_PATH = os.path.join(_base_dir, "ai_team.db")
...
def clear_vault_index():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM vault_tags")
    cursor.execute("DELETE FROM vault_links")
    cursor.execute("DELETE FROM vault_nodes")
    conn.commit()
    conn.close()
